...
def generate_reports_and_cache():
    ...
    # Les définir comme "Daemons" (ils mourront avec le programme principal)
    t1.daemon = True
    t2.daemon = True
    t3.daemon = True
...
counter_highest_spenders = Counter('highest_spenders', 'Total calls to highest spenders report')
counter_best_sellers = Counter('best_sellers', 'Total calls to best sellers report')
